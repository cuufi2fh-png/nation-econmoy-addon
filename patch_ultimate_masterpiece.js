...
const fullCode = `import { world, system } from "@minecraft/server";
    ...
    if (item.typeId.includes("banner")) {
        ...
        system.run(() => {
            ...
            .show(player).then(res => {
                ...
                const reData = {
                    id: "re_" + Date.now(),
                    name: name, price: price, owner: player.name,
...
system.runInterval(() => {
    const allFlagpolesStr = world.getDynamicProperty("all_flagpoles");
    if (allFlagpolesStr) {
        const allFlagpoles = JSON.parse(allFlagpolesStr); const counts = {};
        for (const nId of Object.values(allFlagpoles)) counts[nId] = (counts[nId] || 0) + 1;
        const overworld = world.getDimension("overworld");
        for (const [nId, count] of Object.entries(counts)) {
            const curr = getCurrencyInfo(nId); const taxAmount = Math.ceil((count * 100) / curr.rate);
            const displayNationName = nId.replace("nation_id_", "국가");
            try {
                overworld.runCommand(\`scoreboard players add "\${displayNationName}" treasury \${taxAmount}\`);
                overworld.runCommand(\`tellraw @a[tag=\${nId}] {"rawtext":[{"text":"§e[국세청] §f영토 \${count}개 세금 §a\${taxAmount.toLocaleString()} \${curr.symbol}§f 징수됨."}]}\`);
            } catch (e) {}
        }
    }
...
fs.writeFileSync('BP/scripts/main.js', fullCode);
console.log("Successfully deployed the ultimate flawless masterpiece main.js!");
