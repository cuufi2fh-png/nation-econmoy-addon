...
world.beforeEvents.itemUse.subscribe((event) => {
    ...
    server_treasury: 0    // 서버 세금 보관용
        ...
        system.run(() => {
            ...
            // 플레이어가 소유한 부동산 찾기
            const myEstates = estates.filter(e => e.owner === player.name);
            
            if (myEstates.length === 0) {
                player.sendMessage("§c소유 중인 부동산이 없습니다.");
                return;
            }
            
            // 부동산 목록 표시
            const form = new ActionFormData().title("§l부동산 목록");
            let bodyText = "§e당신의 부동산 목록:\n\n";
            for (const e of myEstates) {
                bodyText += `§a${e.name} - ${e.price.toLocaleString()}₩\n`;
            }
            form.body(bodyText);
            ...
            form.show(player).then(res => {
                if (res.canceled || res.selection === myEstates.length) return;
                
                const selected = myEstates[res.selection];
                const relatedTrans = transactions.filter(t => t.estate === selected.id);
                
                // 부동산 상세 정보 표시
                let detailText = `§l부동산 상세 정보\n\n`;
                detailText += `§a물건명: ${selected.name}\n`;
                detailText += `§b현재 소유자: ${selected.owner}\n`;
                detailText += `§c기본 가격: ${selected.price.toLocaleString()}₩\n`;
                if (selected.type === 0) detailText += `§e유형: 주택 매매\n`;
                else if (selected.type === 1) detailText += `§e유형: 주택 임대\n`;
                else if (selected.type === 2) detailText += `§e유형: 상업용 건물\n`;
                else if (selected.type === 3) detailText += `§e유형: 건물주 운영 중\n`;
                
                detailText += `\n§f거래 기록 (${relatedTrans.length}건):\n`;
                for (const t of relatedTrans.slice(-3)) {
                    detailText += `§7- ${t.buyer}가 ${t.basePrice.toLocaleString()}₩에 구매\n`;
                }
                
                new ActionFormData().title("§l부동산 상세 정보").body(detailText).button("닫기").show(player);
            });
        });
    }
});
...
function showRealEstateTradingUI(player, estate, estates, bIdx) {
    const curr = getCurrencyInfo(getNationId(player));
    let money = 0;
    try { money = world.scoreboard.getObjective("player_money").getScore(player); } catch(e){}
    
    // 매매 거래
    if (estate.type === 0) { // 주택 매매
        const trans = calculateRealEstateFee(estate.price, getNationId(player));
        const bodyText = `🏠 물건: ${estate.name}\n💰 가격: ${estate.price.toLocaleString()}${curr.symbol}\n💳 수수료: ${trans.fee.toLocaleString()}${curr.symbol} (5.14%)\n🏪 총액: ${trans.totalPayment.toLocaleString()}${curr.symbol}\n👑 판매자: ${estate.owner}`;
        
        new ActionFormData().title("🏠 주택 매매").body(bodyText)
        .button("§a구매").button("취소").show(player).then(r => {
            if (r.canceled || r.selection === 1) return;
            if (money < trans.totalPayment) { player.sendMessage("§c잔액이 부족합니다."); return; }
            
            processPropertySale(player, estate, player.name, estate.owner, estate.price);
            estates[bIdx].owner = player.name;
            world.setDynamicProperty("real_estates", JSON.stringify(estates));
            player.runCommand("give @s nf:building_cert 1");
            player.sendMessage("§a[부동산] 주택 매매 완료! 실소유증명서가 발급되었습니다.");
        });
    }
    // 주택 임대
    else if (estate.type === 1) { 
        if (estate.tenant === player.name) { player.sendMessage(`§e이미 임대 중입니다. (월세: ${estate.price.toLocaleString()}₩)`); return; }
        if (estate.tenant !== "") { player.sendMessage(`§c이미 임대된 주택입니다.`); return; }
        
        new ActionFormData().title("🏠 주택 임대").body(`물건: ${estate.name}\n월세: ${estate.price.toLocaleString()}${curr.symbol}\n(48분마다)\n집주인: ${estate.owner}`)
        .button("§a임대 계약").button("취소").show(player).then(r => {
            if (r.canceled || r.selection === 1) return;
            ...
            world.setDynamicProperty("real_estates", JSON.stringify(estates));
            player.sendMessage("§a[부동산] 임대 계약 완료!");
        });
    }
    // 상업용 건물 매매
    else if (estate.type === 2) {
        const trans = calculateRealEstateFee(estate.price, getNationId(player));
        const bodyText = `🏢 건물: ${estate.name}\n💰 가격: ${estate.price.toLocaleString()}${curr.symbol}\n💳 수수료: ${trans.fee.toLocaleString()}${curr.symbol}\n🏪 총액: ${trans.totalPayment.toLocaleString()}${curr.symbol}`;
        
        new ActionFormData().title("🏢 건물 매매").body(bodyText)
        .button("§a인수").button("취소").show(player).then(r => {
            if (r.canceled || r.selection === 1) return;
            if (money < trans.totalPayment) { player.sendMessage("§c잔액이 부족합니다."); return; }
            ...
            world.setDynamicProperty("real_estates", JSON.stringify(estates));
            player.runCommand("give @s nf:building_cert 1");
            player.sendMessage("§a[부동산] 건물 인수 완료! 건물 증명서와 실소유증명서가 발급되었습니다.");
        });
    }
}
...
world.beforeEvents.playerInteractWithBlock.subscribe((event) => {
    ...
    // 카지노 머신 클릭
    if (block.typeId === "nf:casino_machine") {
        event.cancel = true;
        
        system.run(() => {
            // 칩 투입 확인
            if (!item || (!item.typeId.includes("chip_1k") && !item.typeId.includes("chip_10k"))) {
                player.sendMessage("§c[카지노] 칩을 들고 클릭하세요.");
                return;
            }
            
            // 칩 가치 계산
            const chipValue = item.typeId === "nf:chip_1k" ? 1000 : 10000;
            const chipAmount = item.amount;
            const totalValue = chipValue * chipAmount;
            
            // 현재 머신 상태 확인
            const machineKey = `casino_${block.location.x}_${block.location.y}_${block.location.z}`;
            let machineData = JSON.parse(world.getDynamicProperty(machineKey) || "null");
            
            // 새로운 베팅이면 기존 베팅 확인
            if (machineData && machineData.waiting) {
                player.sendMessage("§c[카지노] 다른 플레이어가 도박 중입니다.");
                return;
            }
            ...
            // 머신 상태 저장 (베팅 금액, 플레이어, 대기 상태)
            machineData = {
                balance: (machineData?.balance || 0) + totalValue,
                owner: player.name,
                waiting: true,
                betTime: Date.now()
            };
            world.setDynamicProperty(machineKey, JSON.stringify(machineData));
            
            // ActionBar에 금액 표시
            player.onScreenDisplay.setActionBar(`§a대기금액: §f${machineData.balance.toLocaleString()}₩`);
            player.sendMessage(`§a[카지노] ${totalValue.toLocaleString()}₩ 베팅했습니다. 버튼을 누르세요!`);
        });
        return;
    }
});

// 카지노 머신 버튼/레버 근처 레드스톤 신호 감지 및 도박 처리
system.runInterval(() => {
    const overworld = world.getDimension("overworld");
    
    // 설치된 모든 카지노 머신 검사 (저부하 - 10틱마다, 짧은 범위)
    const machines = overworld.getBlocks({ x: -999999, y: -64, z: -999999 }, { x: 999999, y: 320, z: 999999 }, { 
        matches: ["nf:casino_machine"] 
    });
    
    for (const blockLocation of machines) {
        const machineKey = `casino_${blockLocation.x}_${blockLocation.y}_${blockLocation.z}`;
        let machineData = JSON.parse(world.getDynamicProperty(machineKey) || "null");
        
        if (!machineData || !machineData.waiting) continue;
        
        // 머신 위 버튼 레드스톤 신호 감지 (y+1에 버튼이 있다고 가정)
        const buttonAbove = overworld.getBlock({ x: blockLocation.x, y: blockLocation.y + 1, z: blockLocation.z });
        const buttonAbove2 = overworld.getBlock({ x: blockLocation.x + 1, y: blockLocation.y, z: blockLocation.z });
        const hasSignal = (buttonAbove && buttonAbove.typeId === "minecraft:redstone_block") || 
                         (buttonAbove2 && buttonAbove2.typeId === "minecraft:redstone_block");
        
        if (!hasSignal) continue;
        
        // 도박 처리
        const player = world.getPlayers({ name: machineData.owner })[0];
        if (!player) {
            world.setDynamicProperty(machineKey, JSON.stringify({ ...machineData, waiting: false }));
            continue;
        }
        ...
        if (isWin) {
            // 승리: 2배 지급
            const winAmount = balance * 2;
            try {
                player.dimension.runCommand(`scoreboard players add "${player.name}" player_money ${winAmount}`);
            } catch(e) {}
            
            player.dimension.runCommand(`playsound levelup @s`);
            player.dimension.runCommand(`playsound pling @s`);
            player.sendMessage(`§a§lJACKPOT! §f${winAmount.toLocaleString()}₩ 획득!`);
            player.onScreenDisplay.setActionBar(`§a§lJACKPOT! +${winAmount.toLocaleString()}₩`);
        } else {
            // 패배: 돈 잃음
            player.dimension.runCommand(`playsound villager.no @s`);
            player.dimension.runCommand(`playsound bass @s`);
            player.sendMessage(`§c꽝! ${balance.toLocaleString()}₩ 잃었습니다.`);
            player.onScreenDisplay.setActionBar(`§c꽝! -${balance.toLocaleString()}₩`);
        }
        
        // 머신 상태 초기화
        world.setDynamicProperty(machineKey, JSON.stringify({ balance: 0, owner: "", waiting: false }));
    }
}, 10);
...
// 카지노 머신 파괴 시 정리
world.afterEvents.playerBreakBlock.subscribe((event) => {
    if (event.brokenBlockPermutation.type.id === "nf:casino_machine") {
        const machineKey = `casino_${event.block.location.x}_${event.block.location.y}_${event.block.location.z}`;
        const machineData = JSON.parse(world.getDynamicProperty(machineKey) || "null");
        
        // 베팅 중이었다면 돈 반환
        if (machineData && machineData.balance > 0 && machineData.owner) {
            const player = world.getPlayers({ name: machineData.owner })[0];
            if (player) {
                player.dimension.runCommand(`scoreboard players add "${machineData.owner}" player_money ${machineData.balance}`);
                player.sendMessage(`§a[카지노] 머신이 파괴되어 베팅금 ${machineData.balance.toLocaleString()}₩가 반환되었습니다.`);
            }
        }
        
        world.setDynamicProperty(machineKey, "null");
    }
});
